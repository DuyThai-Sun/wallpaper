...
final class DetailViewController: UIViewController {
    ...
    private func saveToCoreData () {
        coreData.addToCoreData(data: dataToSave) { [unowned self] error in
            showPopUp(notice: error ?? "")
        }
    }
    
    private func checkIsFavorited() {
        if coreData.checkInCoreData(id: id) {
            isFavorited = true
    ...
    @IBAction func favoriteButtonTapped(_ sender: Any) {
        if isFavorited {
            coreData.removeCoreData(id: id) {[unowned self] error in
                showPopUp(notice: error ?? "")
            }
            favoriteButton.tintColor = .white
            isFavorited = false
    ...
    private func imageToCoreDataObject(image: Image) -> CoreDataObject{
        let data = CoreDataObject(id: image.id, width: image.width,
                                  height: image.height, url: image.source.portrait, photographer: image.photographer, photographerId: image.photographerId, avgColor: image.avgColor, isVideo: 0, videoDuration: nil)
        return data
    }
    ...
    private func videoToCoreDataObject(video: Video) -> CoreDataObject{
        let data = CoreDataObject(id: video.id, width: video.width, height: video.height, url: video.videoFiles[0].link, photographer: video.user.name, photographerId: video.user.id, avgColor: "", isVideo: 1, videoDuration: video.duration)
        return data
    }
    ...
    private func informationFromCoreData(data: CoreDataObject) {
        ...
        imageIdLabel.text = "Id: \(data.id ?? 0)"
        colorImageLabel.text = "Color: no"
    }
